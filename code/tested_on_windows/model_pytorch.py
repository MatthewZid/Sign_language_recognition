...
class LandmarksDataset(Dataset):
    ...
    def __init__(self, csv_words_paths, landmarks_csvs_dir, transform=None):
        ...
        self.max_len_frame = int((self.landmarks_dir.split('\\'))[-1].split('_')[-2])
        self.individual_words = self.words_paths.columns
        self.individual_words = self.individual_words.to_list()
        self.words_to_numbers = np.arange(len(self.individual_words)).tolist() # The representation of each word as a number is just its index in individual_words list
        count_samples = []
    ...
    def __getitem__(self, idx):
        ...
        if abs(self.max_len_frame - landmarks.shape[1]) != 0:
            initial_shape = landmarks.shape[1] #If the the shape[1] is 0 then the array should be converted to np.float
            landmarks = np.append((landmarks[0]),
                                  [[0] * landmarks.shape[2] for _ in range(self.max_len_frame - landmarks.shape[1])],
                                  axis=0)
            landmarks = np.array([landmarks])

            if initial_shape == 0:
                temp = np.vstack(landmarks[:, :, :]).astype(np.float)
                landmarks = np.expand_dims(temp, axis=0)
        ...
        sample = {'landmarks': landmarks, 'word_label': word_number_label}
...
def evaluate(modell, loss_fn, vl_dg, words_to_nums):
    with torch.no_grad():
        ...
        for batch_smpl in vl_dg:
            ...
            yb_all.append(yb)
            results = [loss_batch(modell, loss_fn, torch.squeeze((torch.from_numpy(np.asarray(batch_smpl['landmarks']))).to(device), 1),
                                  yb,
                                  words_to_nums
                                  )
                       ]
...
def classific_report(y_true, y_pred, probabilities, indiv_words, words_to_num, epch, epchs, earl_stop, pth_results, pth_txt, dataset_type="val"):
    ...
    clas_report = classification_report(y_true, y_pred, target_names=indiv_words, labels=range(len(indiv_words)), zero_division=0)
...
if __name__ == '__main__':
    ...
    for batch_sample in train_landmarks_dataset:
        num_features = list(batch_sample['landmarks'].size())[2]
        window_size = list(batch_sample['landmarks'].size())[1]

        break

    learning_rate = 0.001
    input_sz = num_features
    hidden_size = 100
    nm_layers = 1
    ...
    # Instantiate Network
    model = Network(input_sz, nm_classes, hidden_size, nm_layers)
    if model.cnn_flag:
        model = model.double()
    ...
    patience = 10
    epochs_no_improve = 0
    early_stop = False #At first it should be False
    min_delta = 0.0001
    ...
    model_summary = summary(model, input_size=(batch_size, list(batch_sample['landmarks'].size())[1], list(batch_sample['landmarks'].size())[2]))
    ...
    param_defs = 'Using: Criterion=' + criterion_to_str + ' Optimizer=' + optimizer_to_str + ' with learning rate=' + str(learning_rate) + '\n'
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < min_val_loss:
            ...
            torch.save(model.state_dict(model.state_dict()), path_save_model + '\epoch_' + str(str_epoch) + '\model.pth')
            ...
            if epochs_no_improve == patience:
                ...
                print_out = 'Early stopping raised at epoch: ' + str(epoch+1) + '. Best model found in epoch: ' + str(epoch+1-patience) + ' with validation loss: ' + str(epoch_val_losses[epoch-patience])
                print(print_out)
                ...
                model.load_state_dict(torch.load(path_save_model + '/epoch_' + str(epoch + 1 - patience) + '\model.pth'))
        ...
        if (epoch+1) % 1 == 0 or early_stop:
            ...
            if isinstance(criterion, type(nn.MultiLabelSoftMarginLoss())) or isinstance(criterion, type(nn.CrossEntropyLoss())):
                y_eval_predicted = [(torch.max(torch.exp(batch_preds[0]), 1)[1]).data.cpu().numpy() for batch_preds in preds_eval] # Returns the maximum values if the predictions
                ...
                list_probs = [np.reshape(((F.softmax(tens, dim=len(list(tens.size()))-1)).numpy()), (-1, nm_classes)) for tens in tensor_preds_eval_plus_last_batch]
            ...
            roc_auc_scs, f1_macr = classific_report(y_eval_all, y_eval_predicted, probs, individual_words, words_to_numbers, epoch+1, epochs, early_stop, path_results, path_txt)
    ...
    create_losses_plot(epoch+1, avg_train_losses, epoch_val_losses, criterion_to_str, path_results) #Plot training and validation losses
    ...
    if isinstance(criterion, type(nn.MultiLabelSoftMarginLoss())) or isinstance(criterion, type(nn.CrossEntropyLoss())):
        y_test_predicted = [(torch.max(torch.exp(batch_preds[0]), 1)[1]).data.cpu().numpy() for batch_preds in preds_test]  # Returns the maximum values if the predictions
                                                                                                                            # are of the form (Batches, Batch_size, Number_of_Classes)
                                                                                                                            # This happens if there is not Softmax output layer
        ...
        # Then, convert the resulting torches to a list of numpys
        # Next, reshape numpy in order not to take into account the number of batches
        test_list_probs = [np.reshape(((F.softmax(tens, dim=len(list(tens.size())) - 1)).numpy()), (-1, nm_classes)) for tens in tensor_preds_test_plus_last_batch]
    ...
    roc_auc_scs, f1_macr = classific_report(y_test_all, y_test_predicted, test_probs, individual_words, words_to_numbers, epoch + 1, epochs, early_stop, path_results, path_txt, 'test')
